...
class NoteMinimap extends Plugin {
    ...
    async onload() {
        ...
        this.registerDomEvent(document, "click", (event) => {
            ...
            if (target.closest(".callout-title, .collapse-indicator, .task-kanban-inline-action")) {
    ...
    async updateElementMinimap(element, helperLeafId) {
        ...
        helperLeafId = activeLeaf ? this.helperLeafIds.get(activeLeaf.id) : helperLeafId;
    ...
    checkAndDealWithUserOpeningHelperLeaves(newActiveLeaf) {
        ...
        if (newActiveLeaf?.id && [...this.helperLeafIds.values()].includes(newActiveLeaf.id))
...
class Minimap {
    ...
    updateSettings(settings) {
        ...
        this.effectiveScaleY = this.scale;
    ...
    updateSettingsInCSS() {
        ...
        if (this.iframe) this.iframe.style.setProperty("--scale-y", this.effectiveScaleY || this.scale);
    ...
    resize(fullHeight, visibleHeight) {
        ...
        this.effectiveScaleY = Math.min(this.scale, this.visibleHeight / this.fullHeight);
        this.updateSettingsInCSS();
        this.iframe.style.height = `${fullHeight}px`;
        this.slider.style.height = `${visibleHeight * this.effectiveScaleY}px`;
        this.updateSliderScroll();
    }
    ...
    updateSliderScroll() {
        ...
        const boxTop = scrollTop * (this.effectiveScaleY || this.scale) + (this.topOffset || 0);
    ...
    onSliderMouseMove = (e) => {
        ...
        const scaleY = this.effectiveScaleY || this.scale;
        const maxOffset = maxScroll * scaleY;

        offsetY = Math.max(0, Math.min(offsetY, maxOffset));

        const scrollY = offsetY / scaleY;
