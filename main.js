...
class Minimap {
    ...
    constructor(plugin, element, settings, helperLeafId) {
        ...
        this.container.addEventListener("wheel", this.onMinimapWheel, { passive: false });
    }
    ...
    syncTaskKanbanCollapse(noteContent) {
        ...
        const kanbanCallouts = Array.from(noteContent.querySelectorAll(".callout")).filter((callout) => {
            const dataCallout = callout.getAttribute("data-callout") || "";
            return dataCallout === "todo"
                || dataCallout === "task-kanban"
                || !!callout.querySelector(".task-kanban-inline-marker, .task-kanban-inline-card")
    ...
    onSliderMouseMove = (e) => {
        ...
        const scrollY = offsetY / maxOffset * maxScroll;
