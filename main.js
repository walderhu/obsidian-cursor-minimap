...
class MinimapController {
  ...
  measureLineWeights(lines) {
    ...
    const weights = [];
    const previewKanbanHeight = this.getRenderedTaskKanbanHeight();
    ...
    for (const line of lines) {
      ...
      if (text.includes("<!-- task-kanban:end -->")) {
        ...
        weights.push(...this.measureKanbanBlockWeights(kanbanLines, baseLineHeight, kanbanCollapsed, previewKanbanHeight));
        insideKanban = false;
  ...
  measureKanbanBlockWeights(lines, baseLineHeight, collapsed, renderedHeight) {
    if (this.isPreviewScroller() && Number.isFinite(renderedHeight) && renderedHeight > 0) {
      const total = collapsed ? Math.max(baseLineHeight * 1.8, renderedHeight) : Math.max(baseLineHeight * 2.2, renderedHeight);
      const weights = lines.map((line) => {
        const text = String(line || "");
        if (text.includes("<!-- task-kanban:start -->") || text.includes("<!-- task-kanban:end -->")) return 0.05;
        if (/\[!(?:todo|task-kanban)\]\+?\s+Task Kanban/.test(text)) return 1.4;
        if (/task-kanban-inline-marker|task-kanban-inline-card/.test(text)) return 5;
    ...
    return lines.map((line) => this.measureKanbanLineWeight(line, baseLineHeight, collapsed));
  }
  ...
  measureKanbanLineWeight(line, baseLineHeight, collapsed) {
    const text = String(line || "");
    if (!this.isPreviewScroller()) {
      if (text.includes("<!-- task-kanban:start -->") || text.includes("<!-- task-kanban:end -->")) return baseLineHeight * 0.2;
      return this.measureWrappedTextWeight(text, baseLineHeight, 80);
    }
    if (collapsed) {
  ...
  getRenderedTaskKanbanHeight() {
    if (!this.isPreviewScroller()) return 0;
    const callout = this.view.containerEl.querySelector(".callout:has(.task-kanban-inline-marker)");
    if (!callout) return 0;
    return callout.getBoundingClientRect().height || callout.scrollHeight || 0;
  }
